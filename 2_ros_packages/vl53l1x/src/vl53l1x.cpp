...
bool Vl53l1x::init()
{
  ...
  // Setup flow control
  int ioctl_result = ioctl(fd, I2C_SLAVE, address);
  ...
  if (address != 0x29) // added by SEKIGUCHI
  {
    std::cout << "escape reset for address change" << std::endl;
    return true;
  }
  ...
  // std::cout << "O_RDWR: " << O_RDWR << ", fd: " << fd << ", address: " << address << I2C_SLAVE << std::endl;
...
void Vl53l1x::getRangingData()
{
  ...
  // std::cout << std::to_string(results.range_status) << std::endl;
