...
def yolo(publisher):
    ...
    # 学習モデルの定義
    model = YOLO("yolo11n.pt")  # poseモデル，xは最も重くて正確
    ...
    results = model(
        source=0, conf=conf, stream=True, max_det=max_det, half=True
    )  # YOLOの実行結果をresultsに格納
    ...
    while True:
        for r in results:
            ...
            degp = [0.0] * num
            keypoints = (
                r.keypoints.xyn.cpu().numpy()
            )  # poseモデル限定，各関節の座標を取得，扱いやすい方に変換
